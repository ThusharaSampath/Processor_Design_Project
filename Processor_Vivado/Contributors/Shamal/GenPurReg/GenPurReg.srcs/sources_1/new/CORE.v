module CORE(
    ...
    always@(*)
     ...
     MDDR MDDR (.clk(clk), .write_ins(wire_decoder_registers[4]),.write_data(wire_decoder_registers[5]),
        .write_bus(wire_decoder_registers[3]),.instr_iram_mddr(wire_iram_mddr),.data_dram_mddr(wire_dram_mddr),
        .data_bus_mddr(bus_wire),.data_out(wire_registers_busMultiplexer[2]));
    ...
    CONTROL_UNIT CONTROL_UNIT(
        ...
        .to_DM(to_DM),
        .to_IM(to_IM),
        .to_Decoder(wire_cu_decoder),
        .to_BUS(bus_selector_from_cu),
        .to_PC(PC_inc_wire),
        .to_AC(ac_cu_wire),
        .en_RegSelector(en_register_selector),
        .rw_RegSelector(rw_register_selector),
        .finish(finish_wire),
        .current_micro_instruction(current_micro_instruction_wire)
    );
